...
impl crate::schemars::JsonSchema for HorizontalPodAutoscalerBehavior {
    ...
    fn json_schema(__gen: &mut crate::schemars::SchemaGenerator) -> crate::schemars::Schema {
        crate::schemars::json_schema!({
            ...
            "properties": {
                "scaleDown": ({
                    let mut schema_obj = __gen.subschema_for::<crate::api::autoscaling::v2::HPAScalingRules>();
                    schema_obj.ensure_object().insert("description".into(), "scaleDown is scaling policy for scaling Down. If not set, the default value is to allow to scale down to minReplicas pods, with a 300 second stabilization window (i.e., the highest recommendation for the last 300sec is used).".into());
                    schema_obj
                }),
                "scaleUp": ({
                    let mut schema_obj = __gen.subschema_for::<crate::api::autoscaling::v2::HPAScalingRules>();
                    schema_obj.ensure_object().insert("description".into(), "scaleUp is scaling policy for scaling Up. If not set, the default value is the higher of:\n  * increase no more than 4 pods per 60 seconds\n  * double the number of pods per 60 seconds\nNo stabilization is used.".into());
                    schema_obj
                }),
            },
        })
