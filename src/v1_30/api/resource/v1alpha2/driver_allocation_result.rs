...
impl crate::schemars::JsonSchema for DriverAllocationResult {
    ...
    fn json_schema(__gen: &mut crate::schemars::SchemaGenerator) -> crate::schemars::Schema {
        crate::schemars::json_schema!({
            ...
            "properties": {
                "namedResources": ({
                    let mut schema_obj = __gen.subschema_for::<crate::api::resource::v1alpha2::NamedResourcesAllocationResult>();
                    schema_obj.ensure_object().insert("description".into(), "NamedResources describes the allocation result when using the named resources model.".into());
                    schema_obj
                }),
                "vendorRequestParameters": ({
                    let mut schema_obj = __gen.subschema_for::<crate::apimachinery::pkg::runtime::RawExtension>();
                    schema_obj.ensure_object().insert("description".into(), "VendorRequestParameters are the per-request configuration parameters from the time that the claim was allocated.".into());
                    schema_obj
                }),
            },
        })
